  ...
  const uploadAll = async () => {
    ...
    for (let i = 0; i < queue.length; i++) {
      ...
      try {
        ...
        setQueue((q) => q.map((it, idx) => idx === i ? { ...it, status: 'error', error: err.message } : it));
      }
    }
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
          ...
          >
            ...
            <p className="text-[11px] text-slate-400">JPG, PNG, WEBP, MP4</p>
          </div>
          <input
            ref={inputRef}
            type="file"
                  ...
                  {item.status === 'pending' && (
                    <button
                      ...
                      className={[
                        ...
                        item.compressed ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-500',
                      ].join(' ')}
                    >
