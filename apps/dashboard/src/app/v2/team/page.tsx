...
function InviteModal({ onClose, onSent }: { onClose: () => void; onSent: () => void }) {
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    try {
      ...
      setInviteUrl(res.inviteUrl); setDone(true); onSent();
    } catch (err: any) { setError(err.message ?? "Failed to send invite"); }
    setLoading(false);
  }
      ...
      <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl overflow-hidden">
        ...
        {!done ? (
          <form onSubmit={handleSend} className="p-6 space-y-4">
            ...
            <div className="flex gap-3 pt-1">
              <button type="button" onClick={onClose}
                className="flex-1 h-10 rounded-lg border border-slate-200 text-[13.5px] font-medium text-slate-600 hover:bg-slate-50">Cancel</button>
              <button type="submit" disabled={loading}
                className="flex-1 h-10 rounded-lg bg-[#0B1F14] text-[13.5px] font-semibold text-white hover:bg-[#1A3525] disabled:opacity-60 flex items-center justify-center gap-2">
          ...
          <div className="p-6 text-center">
            ...
            <p className="text-[13px] text-slate-500 mb-4">An invite link was sent to <strong>{email}</strong>.</p>
            {inviteUrl && (
              <div className="rounded-lg bg-slate-50 border border-slate-200 p-3 text-left mb-4">
                <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-1">🛠 Dev — invite link</p>
                <p className="text-[12px] text-slate-700 break-all">{inviteUrl}</p>
                <button onClick={() => navigator.clipboard.writeText(inviteUrl)}
                  className="mt-2 text-[11.5px] text-emerald-700 font-medium hover:underline">Copy link</button>
              </div>
            )}
            <button onClick={onClose}
              className="w-full h-10 rounded-lg bg-[#0B1F14] text-[13.5px] font-semibold text-white hover:bg-[#1A3525]">Done</button>
          </div>
        )}
      </div>
    </div>
...
export default function TeamPage() {
  ...
  // KEY FIX: derive isOwner from the members API response, not the JWT.
  // The JWT role field may be stale or missing — the members list is authoritative.
  ...
  async function load() {
    ...
    const [m, i] = await Promise.all([
      ...
      apiGet<Invite[]>("/team/invites").catch(() => []),
    ]);
    setMembers(m);
    setInvites(i);
    setLoading(false);
  ...
  async function handleRoleChange(memberId: string, role: string) {
    try {
      ...
      setMembers((prev) => prev.map((m) => m.memberId === memberId ? { ...m, role: role as any } : m));
    } catch (e: any) { alert(e.message); }
  }
  ...
  async function handleResend(inviteId: string) {
    ...
    try {
      await apiPost(`/team/invites/${inviteId}/resend`, {});
      setResentId(inviteId);
      setTimeout(() => setResentId(null), 2000);
    } catch (e: any) { alert(e.message ?? "Failed to resend"); }
    setResending(null);
  }
    ...
    <PageContainer className="bg-[#F7F5F0]">
      ...
      <div className="rounded-2xl bg-white border border-slate-100 shadow-[0_1px_3px_rgba(0,0,0,0.06)] overflow-hidden">
        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-3.5">
          ...
          <span className="rounded-full bg-slate-100 px-2.5 py-0.5 text-[11px] font-semibold text-slate-500">{members.length}</span>
        </div>
                ...
                <div key={m.memberId} className="flex items-center gap-4 px-5 py-3.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`rounded-full px-2 py-0.5 text-[10.5px] font-semibold ${ROLE_BADGE[m.role]}`}>{m.role}</span>
                    </div>
                  ...
                  {isOwner && !isSelf && (
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      <button onClick={() => handleRemove(m.memberId)} disabled={removing === m.memberId}
                        className="flex h-7 w-7 items-center justify-center rounded-lg border border-slate-200 text-slate-400 hover:border-red-300 hover:text-red-600 transition-all" title="Remove member">
                        {removing === m.memberId ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Trash2 className="h-3.5 w-3.5" />}
                      </button>
                    </div>
                  )}
                </div>
      ...
      {isOwner && (
        <div className="mt-5 rounded-2xl bg-white border border-slate-100 shadow-[0_1px_3px_rgba(0,0,0,0.06)] overflow-hidden">
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-3.5">
            ...
            {invites.length > 0 && (
              <span className="rounded-full bg-amber-100 px-2.5 py-0.5 text-[11px] font-semibold text-amber-700">{invites.length}</span>
            )}
          </div>
          {invites.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10 text-center">
              ...
              <p className="text-[12px] text-slate-400 mt-0.5">Invited brokers will appear here until they accept.</p>
            </div>
          ) : (
            <div className="divide-y divide-slate-50">
              {invites.map((inv) => (
                <div key={inv.id} className="flex items-center gap-4 px-5 py-3.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`rounded-full px-2 py-0.5 text-[10.5px] font-semibold ${ROLE_BADGE[inv.role] ?? "bg-slate-100 text-slate-600"}`}>{inv.role}</span>
                    </div>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button onClick={() => handleResend(inv.id)} disabled={resending === inv.id || resentId === inv.id}
                      className="flex items-center gap-1.5 h-7 px-3 rounded-lg border text-[12px] transition-all"
                      style={resentId === inv.id ? { borderColor: '#10b981', color: '#059669', background: '#f0fdf4' } : { borderColor: '#e2e8f0', color: '#64748b' }}>
                      {resending === inv.id ? <Loader2 className="h-3 w-3 animate-spin" />
                        : resentId === inv.id ? <CheckCircle2 className="h-3 w-3" />
                        : <RefreshCw className="h-3 w-3" />}
                      {resentId === inv.id ? "Sent!" : "Resend"}
                    </button>
                    <button onClick={() => handleRevokeInvite(inv.id)} disabled={revoking === inv.id}
                      className="flex items-center gap-1.5 h-7 px-3 rounded-lg border border-slate-200 text-[12px] text-slate-500 hover:border-red-300 hover:text-red-600 transition-all">
                      {revoking === inv.id ? <Loader2 className="h-3 w-3 animate-spin" /> : <XCircle className="h-3 w-3" />}
                      Revoke
                    </button>
                  </div>
                </div>
      ...
      {showModal && <InviteModal onClose={() => setShowModal(false)} onSent={load} />}
    </PageContainer>
  );
}